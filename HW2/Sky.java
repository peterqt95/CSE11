...
public class Sky extends WindowController {
    ...
    public static final int CENTER_SUN_X = 175;
    public static final int RIGHT_CORNER_SUN = 350;
	...
	if((input >= 0 && input < 6) || (input >= 18 && input <= 24)){
	    ...
	    sky.setColor(new Color(0,0,255));
	    FilledOval sun = new FilledOval(CENTER_SUN_X, 0, SUN_DIAMETER, 
					    SUN_DIAMETER, canvas);
	    sun.setColor(new Color(255,255,0));
	}
	else if(input >=14 && input < 18){
	    sky.setColor(new Color(0,0,255));
	    FilledOval sun = new FilledOval(RIGHT_CORNER_SUN, 0,

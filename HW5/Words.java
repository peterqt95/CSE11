...
public class Words {
	...
	//Cycles through until user option 3 is used
	while(!word.equals("3")){
	    //Prints out and stores the available words for you
	    System.out.println("");
	    for(int i = 0; i < MAX_LETTERS; i++){
		temp[i] = myWords[i];
	    ...
	    //Returns new letters for you
	    else if(word.equals("2")){
		for(int i = 0; i < MAX_LETTERS; i++){
		    myWords[i] = getLetter();
		}
	    }
	    //Returns the longest word you can enter
	    ...
	    //Checks to see if your entered word is valid
	    else{
		isPresent = checkLetter(myWords, word);
